...
winner <- function(x,y){
  if(x>y){
    game_score$winner <- c(1,0)
  } else {
    game_score$winner <- c(0,1)
  }
}

  
# Add Win column
for(i in all_gamelogs$game.id){
  game_score <- all_gamelogs %>% 
      filter(game.id == i) %>%
      select(total_pts.points)
  
  lapply(game_score, winner, x = total_points.points[1], y = total_points.points[2])
}
